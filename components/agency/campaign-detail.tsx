...
import { ArrowLeft, ArrowRight, Check, ExternalLink, Mail, RefreshCw, Search, Trash2 } from "lucide-react"
...
export function CampaignDetail({ initial, creditBalance, owner, sender }: { initial: Detail; creditBalance: number; owner: boolean; sender: SenderProfile | null }) {
  ...
  return <main className="min-h-screen bg-muted/40 p-4 text-foreground sm:p-6 lg:p-8">
    <div className="mx-auto max-w-[1400px] space-y-5">
      ...
      <Card><CardHeader><CardTitle>Ready for approval</CardTitle><CardDescription>Generate and open the exact Company Radar PDF before approving any physical mail. A changed letter requires a new preview.</CardDescription></CardHeader><CardContent>{pendingBatches.length ? <div className="space-y-3">{pendingBatches.map((batch) => { const count = detail.mailItems.filter((item) => item.batchId === batch.id).length; const enoughCredits = creditBalance >= count; return <div key={batch.id} className="flex flex-wrap items-center justify-between gap-3 border-2 p-3"><div><p className="font-black">{batch.name}</p><p className="text-sm text-muted-foreground">{count} {count === 1 ? "recipient" : "recipients"} · {count} {count === 1 ? "credit" : "credits"} · A4 single-sided · Second Class</p>{!enoughCredits ? <p className="mt-1 text-xs font-bold text-destructive">Buy {count - creditBalance} more credit{count - creditBalance === 1 ? "" : "s"} to approve this batch.</p> : null}</div>{owner ? <div className="flex gap-2"><Button variant="outline" disabled={!approvalSafe || busy !== null} onClick={() => preview(batch.id)}>Open exact PDF</Button><Button disabled={!approvalSafe || !enoughCredits || busy !== null} onClick={() => approve(batch.id)}>Approve & send</Button></div> : null}</div>})}</div> : <p className="border-2 border-dashed p-5 text-sm text-muted-foreground">Select eligible matches to prepare an approval batch.</p>}</CardContent></Card>
...
function formatLondonDateTime(value: string) { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("en-GB", { timeZone: "Europe/London", dateStyle: "short", timeStyle: "short" }).format(date) }
function formatDate(value: string) { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("en-GB", { timeZone: "Europe/London", day: "numeric", month: "short", year: "numeric" }).format(date) }
