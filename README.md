...
Do not use `wrangler deploy` directly unless `npm run cf:build` has already run in the same build job. The deploy step needs the `.open-next/worker.js` output generated by OpenNext.
