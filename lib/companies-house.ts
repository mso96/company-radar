...
export interface CompanyProfile {
  ...
  officers: Array<{ name: string; role: string; appointedOn?: string; resignedOn?: string }> | null
  owners: Array<{ name: string; kind?: string; notifiedOn?: string; ceasedOn?: string }> | null
  shares: { shareCapital: string | null; lastUpdated: string | null } | null
  financials: { accountsNextDue: string | null; lastAccountsMadeUpTo: string | null; filingCount: number } | null
}
...
export async function fetchCompanyProfile(apiKey: string, companyNumber: string): Promise<CompanyProfile> {
  ...
  const [companyPayload, officersPayload, ownersPayload, capitalPayload, filingsPayload] = await Promise.all([
    ...
    fetchOptionalCompaniesHouseDocument(apiKey, `${base}/officers?items_per_page=100`),
    fetchOptionalCompaniesHouseDocument(apiKey, `${base}/persons-with-significant-control?items_per_page=100`),
    fetchOptionalCompaniesHouseDocument(apiKey, `${base}/capital`),
    fetchOptionalCompaniesHouseDocument(apiKey, `${base}/filing-history?items_per_page=1`),
  ])
  ...
  const registeredOffice = office.address_line_1 && office.locality && office.postal_code
    ...
    : null
  const officers = officersPayload ? itemsFrom(officersPayload).map((item) => ({ name: String(item.name ?? "Unknown"), role: String(item.officer_role ?? "officer"), appointedOn: item.appointed_on ? String(item.appointed_on) : undefined, resignedOn: item.resigned_on ? String(item.resigned_on) : undefined })) : null
  const owners = ownersPayload ? itemsFrom(ownersPayload).map((item) => ({ name: String(item.name ?? item.noc_name ?? "Unknown"), kind: item.kind ? String(item.kind) : undefined, notifiedOn: item.notified_on ? String(item.notified_on) : undefined, ceasedOn: item.ceased_on ? String(item.ceased_on) : undefined })) : null
  const shareCapital = capitalPayload && Array.isArray(capitalPayload.share_capital) ? capitalPayload.share_capital.map((item) => { const row = item as Record<string, unknown>; return `${String(row.share_class ?? "Shares")}: ${String(row.total_amount ?? row.number_of_shares ?? "—")}` }).join(" · ") || null : null
  ...
  return { company, registeredOffice, officers, owners, shares: capitalPayload ? { shareCapital, lastUpdated: capitalPayload.last_full_members_list_date ? String(capitalPayload.last_full_members_list_date) : null } : null, financials: filingsPayload ? { accountsNextDue: accounts.next_due ? String(accounts.next_due) : null, lastAccountsMadeUpTo: lastAccounts.made_up_to ? String(lastAccounts.made_up_to) : null, filingCount: Number(filingsPayload.total_count ?? 0) || 0 } : null }
}

async function fetchOptionalCompaniesHouseDocument(apiKey: string, url: string): Promise<Record<string, unknown> | null> {
  try {
    return await fetchCompaniesHouseDocument(apiKey, url)
  } catch {
    // A profile is still useful when an optional Companies House collection is
    // temporarily unavailable. The UI renders these sections as unavailable
    // instead of showing a misleading zero or failing the whole record.
    return null
  }
}
