...
export async function listRadars(db: D1Database, workspaceId: string): Promise<AgencyRadar[]> {
  const rows = await db.prepare(`SELECT r.*, s.sic_code FROM agency_radars r LEFT JOIN agency_radar_sic_codes s ON s.radar_id = r.id WHERE r.workspace_id = ?1 AND r.archived_at IS NULL ORDER BY r.created_at DESC, s.sic_code ASC`).bind(workspaceId).all<RadarRow>()
...
export async function archiveRadar(db: D1Database, workspaceId: string, radarId: string) {
  const now = new Date().toISOString()
  const result = await db.prepare(`UPDATE agency_radars SET is_active=0, archived_at=?1, paused_at=COALESCE(paused_at,?1), updated_at=?1 WHERE id=?2 AND workspace_id=?3 AND archived_at IS NULL`).bind(now, radarId, workspaceId).run() as { meta?: { changes?: number }; changes?: number }
  if (Number(result.meta?.changes ?? result.changes ?? 0) !== 1) throw new Error("Campaign not found or already deleted.")
}
...
export async function cloneTemplateFromLibrary(db: D1Database, workspaceId: string, sourceTemplateId: string) {
  ...
  await db.prepare(`INSERT INTO agency_letter_templates (id, workspace_id, name, subject, body_html, cta_text, cta_url, signature, is_default, created_at, updated_at, source_template_id, segment_slug, template_version, is_platform_template, pricing_version, service_focus_json, layout_json) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, 0, ?9, ?9, ?10, ?11, ?12, 0, ?13, ?14, ?15)`).bind(id, workspaceId, source.name, source.subject, source.body_html, source.cta_text, source.cta_url, source.signature, now, source.id, source.segment_slug, source.version, source.version, source.service_focus_json ?? "[]", source.layout_json).run()
  return id
}
...
function mapTemplateLibrary(row: TemplateLibraryRow): AgencyTemplateLibraryItem { return { id: row.id, segmentSlug: row.segment_slug, name: row.name, description: row.description, subject: row.subject, bodyHtml: row.body_html, ctaText: row.cta_text ?? undefined, ctaUrl: row.cta_url ?? undefined, signature: row.signature, mergeFields: safeJson<string[]>(row.merge_fields_json, []), pricePence: row.price_pence, currency: row.currency, version: row.version, serviceFocus: safeJson<string[]>(row.service_focus_json, []), layout: safeJson(row.layout_json, undefined) } }
