...
import { completeMailBatch, getSenderProfile, listPendingMailItems, refundCredit, reserveCredits, updateFrankkMailItem } from "@/lib/agency/mail"
...
export async function POST(request: Request) {
  try {
    ...
    for (const campaign of campaigns) {
      ...
      await updateFrankkMailItem(db, campaign.itemId, { status: "pending_approval", quotedCostPence: cost.costPerRecipientPence, totalCostPence: cost.totalPence, currency: cost.currency, providerStatus: "Approved" })
    ...
    if (providerBalance < requiredBalance) throw new Error(`Frankk balance is too low. Required £${(requiredBalance / 100).toFixed(2)}, available £${(providerBalance / 100).toFixed(2)}. Customer credits were not changed.`)
    ...
    for (const item of prepared) {
      try {
        ...
        await updateFrankkMailItem(db, item.itemId, { status: "submitted", orderId: scheduled.orderId, providerStatus: "Scheduled", quotedCostPence: item.cost.costPerRecipientPence, totalCostPence: item.cost.totalPence, currency: item.cost.currency, scheduled: true })
        ...
        await updateFrankkMailItem(db, item.itemId, { status: unknown ? "blocked" : "failed", providerStatus: unknown ? "submission_unknown" : "Failed", error: error instanceof Error ? error.message : "Frankk submission failed.", submissionUnknown: unknown })
    ...
    return NextResponse.json({ results, providerBalancePence: providerBalance, requiredProviderCostPence: requiredBalance })
