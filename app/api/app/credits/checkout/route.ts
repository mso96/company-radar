...
export async function POST(request: Request) {
  try {
    ...
    const lineItem = pack.stripePriceId ? { price: pack.stripePriceId, quantity: 1 } : { price_data: { currency: "gbp", unit_amount: pack.pricePence, tax_behavior: "exclusive" as const, product_data: { name: `${pack.credits} UK Company Radar mail credits`, description: "One credit covers one standard single-sided letter." } }, quantity: 1 }
    const checkout = await stripe.checkout.sessions.create({ mode: "payment", line_items: [lineItem], customer_email: session.email, client_reference_id: session.workspaceId, automatic_tax: { enabled: true }, metadata: { kind: "agency_credit", workspace_id: session.workspaceId, pack_id: pack.id, credits: String(pack.credits) }, success_url: `${origin}/app?credits=success`, cancel_url: `${origin}/app?credits=cancelled` })
