...
const mailFeatures = ["Prepaid physical-mail credits", "Address validation before send", "Frankk print and dispatch status", "Automatic refund on confirmed provider failure"]

export default function PricingPage() {
  return <main className="min-h-screen bg-background text-foreground"><div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-7 sm:px-6 lg:px-8"><MarketingHeader /><section className="border-2 bg-card px-5 py-8 shadow-[6px_6px_0_0_hsl(var(--foreground))] sm:px-8"><Badge variant="outline" className="border-2">Agency pricing</Badge><h1 className="mt-5 max-w-3xl text-4xl font-black leading-[.96] sm:text-6xl">Own the next company on your client’s market.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">Agency Mode is currently in private beta. Join the waitlist to get an early-access place.</p></section><section className="grid gap-5 lg:grid-cols-2"><PriceCard icon={<Radar className="size-5" />} name="Agency Mode" description="Build radars and prepare outreach without a platform subscription." price="Free" features={radarFeatures} cta="Join the waitlist" href="/agency#waitlist" /><PriceCard icon={<Mail className="size-5" />} name="Mail credits" description="Prepaid credits are configured in your workspace before checkout." price="Pay as you send" features={mailFeatures} cta="Join the waitlist" href="/agency#waitlist" featured /></section></div></main>
}
